...
export const useFileReader = (options) => {
  ...
  useEffect(() => {
    ...
    if (accept && !accept.includes(fileType)) {
      setError({
        type: "fileType",
        msg: `Invalid file type: ${fileType}. Accepted types: ${accept.join(
          ", "
        )}`,
      });
    }
    ...
    if (maxFileSize && fileSizeMB > maxFileSize) {
      setError({
        type: "fileSize",
        msg: `File size exceeds the maximum limit of ${maxFileSize} MB.`,
      });
    }
    ...
    fr.onerror = (err) => {
      setError({
        type: "fileRead",
        msg: `Error reading file: ${file.name}. ${err.message}`,
      });
    };
